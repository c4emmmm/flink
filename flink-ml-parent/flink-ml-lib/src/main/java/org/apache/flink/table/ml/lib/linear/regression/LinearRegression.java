 ...
 *
 */
public class LinearRegression implements Estimator<LinearRegression, LinearRegressionModel>,
	WithGradientDescentParams<LinearRegression>,
	WithFeatureCol<LinearRegression>,
	WithLabelCol<LinearRegression>,
	WithPredictionCol<LinearRegression> {
	private final GradientDescent estimator =
		new GradientDescent().setGradientType(Gradient.Type.LEAST_SQUARE);

	public LinearRegression() {
	}

	@Override
	public LinearRegressionModel fit(TableEnvironment tEnv, Table input) {
		return new LinearRegressionModel(estimator.fit(tEnv, input));
	}
